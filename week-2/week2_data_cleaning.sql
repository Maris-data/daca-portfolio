-- Nädal 2 Anmdete puhastamine
SELECT
    sale_id,
    COUNT(*) AS koopiate_arv
FROM sales
GROUP BY sale_id -- grupeeri read sale_id kaupa, leian duplkaatsed sale_id väärtused 
...
FROM sales
WHERE customer_id IS NULL;                          --NULL väärtuste leidmiseks kasutad IS NULL (mitte = NULL!)
...
SELECT COALESCE(NULL, NULL, 'Kolmas valik');        --COALESCE võtab mitu argumenti ja tagastab esimese, mis EI OLE NULL
-- Tulemus: 'Kolmas valik'

-- NULLIF(a, b): kui a = b, tagastab NULL; muidu tagastab a, NULLIF võrdleb kahte väärtust. Kui need on võrdsed, tagastab NULL
...
SELECT
    sale_date,
    TO_CHAR(sale_date, 'DD.MM.YYYY') AS eesti_formaat,  --TO_CHAR() muudab kuupäeva soovitud teksti formaati
...
SELECT DISTINCT
    city AS originaal,
    TRIM(city) AS trimitud,                         --TRIM() eemaldab tühikud teksti algusest ja lõpust
    UPPER(TRIM(city)) AS suurtahtedega,             --UPPER() ja LOWER() muudavad teksti suur- või väiketähtedeks
...
SELECT
    ...
    INITCAP(TRIM(city)) AS puhastatud,              --INITCAP() muudab iga sõna esitähe suureks: 'tallinn' -> 'Tallinn', 'new york' -> 'New York'.
...
--ROW_NUMBER() klientide duplikaatide leidmiseks
 SELECT * FROM (
    SELECT
        ...
        last_name,
        email,
        ROW_NUMBER() OVER (PARTITION BY email ORDER BY customer_id) AS rn  --PARTITION BY email grupeerib read sama e-maili järgi ning ORDER BY customer_id tagab, et väikseim ID saab rn=1 (originaal)
    FROM customers
    WHERE email IS NOT NULL                         --WHERE email IS NOT NULL jätab välja read, kus e-mail puudub (need ei ole duplikaadid, vaid puuduvad andmed)
) numbered
WHERE rn > 1                                        --Välimise päringu WHERE rn > 1 filtreerib ainult duplikaadid
ORDER BY email;

--COALESCE puuduvatele andmetele
SELECT
    customer_id,
    COALESCE(first_name, 'Tundmatu') AS eesnimi,  --COALESCE asendab NULL väärtused loetava tekstiga — päringu TULEMUS on puhtam
    COALESCE(last_name, '') AS perenimi,           --Algandmed tabelis EI MUUTU — COALESCE muudab ainult väljundit
    COALESCE(email, 'puudub') AS email,            --COALESCE SELECT-is on diagnostiline tööriist. Andmete tegelikuks parandamiseks kasutatakse UPDATE-i 
    COALESCE(phone, 'puudub') AS telefon,
    city
FROM customers
WHERE first_name IS NULL                           --WHERE filtreerib ainult need kliendid, kellel on vähemalt üks puuduv väli
   OR last_name IS NULL
   OR email IS NULL
   OR phone IS NULL
ORDER BY customer_id;
...
SELECT
    INITCAP(TRIM(city)) AS puhas_linn,          --INITCAP(TRIM(city)) ühtlustab kõik kirjaviisid: 'tallinn', ' Tallinn ', 'TALLINN' -> 'Tallinn'
    COUNT(*) AS kliente,
    COUNT(DISTINCT city) AS erinevaid_kirjaviise,
    STRING_AGG(DISTINCT city, ', ' ORDER BY city) AS algkirjaviisid
FROM customers
WHERE city IS NOT NULL
GROUP BY INITCAP(TRIM(city))                     --COUNT(DISTINCT city) näitab, mitu erinevat algset kirjaviisi sellel linnal on
...
SELECT
    INITCAP(TRIM(city)) AS linn,                --INITCAP(TRIM(city)) ühtlustab kõik kirjaviisid: 'tallinn', ' Tallinn ', 'TALLINN' -> 'Tallinn'
    COUNT(*) AS kliente_kokku,
    COUNT(*) - COUNT(email) AS email_puudub,
    ROUND(
...
FROM customers
GROUP BY INITCAP(TRIM(city))                    --GROUP BY puhastatud versiooni järgi koondab kõik kirjaviisid ühte gruppi
ORDER BY kliente_kokku DESC;

--COUNT(DISTINCT city) näitab, mitu erinevat algset kirjaviisi sellel linnal on
--STRING_AGG näitab kõiki algkirjaviise komadega eraldatult